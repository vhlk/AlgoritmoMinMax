...
class MinMax {
    public static function minimaxDecision(state:Vector<Array<User>>):Int {
		return maxValue(state).column;
	}

	private static function maxValue(state:Vector<Array<User>>):NextMove {
	...
	private static function minValue(state:Vector<Array<User>>):Float {
		var utility = terminalTest(state);
		if (utility != -1)
			return utility;
